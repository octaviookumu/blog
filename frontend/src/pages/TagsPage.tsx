...
import { useTags, useCreateTags, useDeleteTag } from '../hooks/useTags';
...
const TagsPage: React.FC<TagsPageProps> = ({ isAuthenticated }) => {
  const { data: tags = [], isLoading: loading, error: fetchError } = useTags();
  ...
  const isSubmitting = createTagsMutation.isPending;

  const error = fetchError
    ? 'Failed to load tags. Please try again later.'
    : createTagsMutation.isError
      ? 'Failed to create tags. Please try again.'
